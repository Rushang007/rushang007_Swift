...
@IBDesignable class NKTextField: UITextField {
    ...
    @IBInspectable var txtLeftImage : UIImage? {
        didSet {
            self.setNeedsDisplay()
        }
    }
    
    @IBInspectable var txtRightImage : UIImage? {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    override func awakeFromNib() {
        ...
        if (txtLeftImage != nil)
        {
            let imgView = UIImageView(image: txtLeftImage)
        ...
        //setting left image
        if (txtRightImage != nil)
        {
            let imgView = UIImageView(image: txtRightImage)
            imgView.frame = CGRect(x: 0, y: 0, width: CGFloat(imageWidth), height: self.frame.height)
            imgView.contentMode = .center
